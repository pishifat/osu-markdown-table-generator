...
async function generate() {
    const buffer = fs.readFileSync('teams.csv');
    ...
    for (const unsplitRow of data) {
        ...
        const remixers = row[1].split(' ');
        const mappers = row[2].split(' ');
        const storyboarders = row[3].split(' ');

        const users = [remixers, mappers, storyboarders];
